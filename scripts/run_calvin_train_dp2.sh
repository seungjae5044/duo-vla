...
if [[ ! -x "${environment_path}/bin/torchrun" ]]; then
  echo "DP2 train environment is missing; run ./scripts/bootstrap_train_single_gpu_env.sh" >&2
  exit 1
fi
...
if [[ "${help_only}" == "true" ]]; then
  ...
  echo "DP2 CALVIN training requires an explicit --seed in {0,1,2}" >&2
  exit 2
fi
...
/usr/bin/env -i "${canonical_environment[@]}" \
  ...
  'import sys; sys.path.insert(0, sys.argv[1]); from duo_vla.gpu_preflight import require_idle_training_gpus; require_idle_training_gpus((0,1))' \
  "${project_dir}/src"
